...
import hdm.wi.clicker.server.QuizPackage;
...
public class QuizCacheMapper {
	 ...
	 */
	private static QuizCacheMapper QuizPackageMapper = null;
	 ...
	 */
	public static QuizCacheMapper quizPackageMapper() {
	    if (QuizPackageMapper == null) {
	    	QuizPackageMapper = new QuizCacheMapper();
	    }
	...
	/**
	 * Methode um eine beliebige Anzahl an Quizzes in den jeweils jüngsten
	 * Versionen anhand Ihrerer ID's aus der DB auszulesen
	 * 
	 * @param	keys - Primärschlüsselattribut(e) (->DB)
	 * @return	Vector mit Quizzes, die den Primärschlüsselattributen entsprechen
		...
		try{
			...
			while(rs.next()){
				if (firstIt) {
					...
					tmpquiz.setTeacherId(rs.getInt("lecturerid"));
					tmpquiz.setDurationQuestion(rs.getInt("questionduration"));
					tmpquiz.setStartingDate(rs.getInt("startdate"));
					tmpquiz.setStartingTime(rs.getInt("starttime"));
					if(rs.getInt(10) > 0) {
					...
					if(rs.getInt("random") > 0) {
						tmpquiz.setRandomOrder(true);
					} 
					else {
						tmpquiz.setRandomOrder(false);
					}
