...
public class AdminTreeViewModel implements TreeViewModel {
	 ...
	 */
	public AdminTreeViewModel(ControllerAsync verwaltungA) {
		...
		selectionModel.addSelectionChangeHandler(new SelectionChangeEvent.Handler() {
			...
			public void onSelectionChange(SelectionChangeEvent event) {
				...
				if (selection instanceof Teacher) {
					...
					setSelectedLecturer((Teacher)selection);
				} 
	 ...
	 */
	public void setSelectedLecturer(Teacher lecturer) {
	...
	public <T> NodeInfo<?> getNodeInfo(T value) {
		...
		if (value instanceof String && (String) value == "Lehrende") {
			...
			lecturerDataProvider = new ListDataProvider<Teacher>();

			verwaltung.getAllTeachers(new AsyncCallback<Vector<Teacher>>() {
				public void onFailure(Throwable caught) {
					Window.alert(caught.getMessage());
				}

				public void onSuccess(Vector<Teacher> lecturers) {
					for (Teacher lecturer : lecturers) {
						lecturerDataProvider.getList().add(lecturer);
					}
				}
			});

			return new DefaultNodeInfo<Teacher>(lecturerDataProvider, new LecturerCell(), selectionModel, null);
		}
	 ...
	 */
	public void updateLecturer(Teacher lecturer) {
		List<Teacher> lecturerList = lecturerDataProvider.getList();
		int i = 0;
		for (Teacher a : lecturerList) {
	 ...
	 */
	public void loeschenLecturer(Teacher lecturer) {

		int i = 0;

		for (Teacher l : lecturerDataProvider.getList()) {
