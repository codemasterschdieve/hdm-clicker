...
public class PieChart extends GChart {
	...
	public PieChart(InfoGraph ci) {
		
		double entireResults = ci.getWrongs() + ci.getCorrectAnswers();
		
		double corrects = (double)Math.round((ci.getCorrectAnswers() / entireResults) *100) /100; 
		...
		int count = 0;
		int lastIndex = 0;
		for(int i = 0; i < ci.getQuestionText().length(); i++) {
			count++;
			if (count == intervall) {
				sb.append(ci.getQuestionText().substring(lastIndex, i+1)+"<br>");
				lastIndex = i+1;
				count = 0;
			}
		}
		if (ci.getQuestionText().length() % 30 != 0) {
			sb.append(ci.getQuestionText().substring(lastIndex));
		}
