...
public class QuizForm extends VerticalPanel {
	 ...
	 */	
	public QuizForm(ControllerAsync verwaltungA) {
	...
	public void fillForm() {
		...
		if (shownQuiz.getStartingDate() > 0 && shownQuiz.getStartingTime() > 0) {
			this.datepicker.setValue(DateTimeFormat.getFormat("yyyyMMdd").parse(new Integer(shownQuiz.getStartingDate()).toString()));
			this.datepicker.setCurrentMonth(DateTimeFormat.getFormat("yyyyMMdd").parse(new Integer(shownQuiz.getStartingDate()).toString()));
			this.startHourIntBox.setValue(shownQuiz.getStartingTime() / 60);
			this.startMinutesIntBox.setValue(shownQuiz.getStartingTime() % 60);
		}
	...
	public void aendernMaske() {
		...
		speichernAnlegenButton.addClickHandler(new ClickHandler() {
			public void onClick(ClickEvent event) {
				...
				shownQuiz.setRandomOrder(randomCheckBox.getValue());
				if (datepicker.getValue() != null) {
					shownQuiz.setStartingDate(new Integer(DateTimeFormat.getFormat("yyyyMMdd").format(datepicker.getValue())));
					shownQuiz.setStartTime((startHourIntBox.getValue()*60)+(startMinutesIntBox.getValue()));
				}
				
				
				verwaltung.editQuiz(shownQuiz, questionVector, new AsyncCallback<Quiz>() {
					public void onFailure(Throwable caught) {
						Window.alert(caught.getMessage());

						verwaltung.getQuiz(shownQuiz, new AsyncCallback<Vector<Quiz>>() {
							public void onFailure(Throwable caught) {
		...
		loeschenButton.addClickHandler(new ClickHandler() {
			public void onClick(ClickEvent event) {
				...
				verwaltung.deleteQuiz(shownQuiz, new AsyncCallback<Void>() {
					public void onFailure(Throwable caught) {
						Window.alert(caught.getMessage());
						speichernAnlegenButton.setEnabled(true);
						loeschenButton.setEnabled(true);
	...
	public void anlegenMaske() {
		...
		speichernAnlegenButton.addClickHandler(new ClickHandler() {
			public void onClick(ClickEvent event) {
				...
				if (datepicker.getValue() != null) {
					verwaltung.createQuiz(passwortTextBox.getText(), buttonDurationIntBox.getValue(), descTextBox.getText(), questionDurationIntBox.getValue(),
	...
	public void ladenQuestions() {
		verwaltung.getAllQuestionsByQuiz(shownQuiz, new AsyncCallback<Vector<Question>>() {
	...
	public void questionsAnzeigen() {
		...
		if ((questionVector != null)	&& (questionVector.size() > 0)) {
			...
			for (Question q : questionVector) {
				...
				questionFlexTable.setWidget(row, 1, new Label(q.getQuestionText()));
	...
	public void ladenCategories() {
		...
		verwaltung.getAllCategoriesByTeacher(new AsyncCallback<Vector<Category>>() {
			public void onFailure(Throwable caught) {
				Window.alert(caught.getMessage());
			}
			...
			public void onSuccess(Vector<Question> result) {
				...
				if (result != null & result.size() > 0) {
					...
					for (Question question : result) {
						...
						if (question.getQuestionText().length() > 30) {
							questionListBox.addItem(question.getQuestionText().substring(0, 30));
						}
						else {
							questionListBox.addItem(question.getQuestionText());
						}
