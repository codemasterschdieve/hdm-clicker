...
public class DBConnection {
	 ...
	 */
	private static Vector<Connection> connection = new Vector<Connection>();
	...
	public static Connection connection() throws RuntimeException {
		try {
			if ( connection.elementAt(i) == null || connection.elementAt(i).isClosed() || connection.elementAt(i).isValid(0)) {
				try {
					DriverManager.registerDriver(new AppEngineDriver());
					
					connection.setElementAt(DriverManager.getConnection(url), i);
				} 
				catch (SQLException e1) {
					connection.setElementAt(null, i);
		...
		Connection c;
		try {
			c = connection.elementAt(i);
			i++;
			if (connection.size() <= i) {
				i = 0;
			}
		} catch (Exception e) {
	...
	public static void openOneOfTenConnection() throws RuntimeException {
		try {
			if (connection.size() < 10) {
				try {
					DriverManager.registerDriver(new AppEngineDriver());
					connection.add(DriverManager.getConnection(url));
				} 
				catch (SQLException e1) {
					connection = null;
