...
public class ChooseQuizFormMobile extends ScrollPanel {
	 ...
	 */	
	public ChooseQuizFormMobile(ControllerAsync verwaltungA) {
	...
	public void ladenQuizze() {
		reqAnswered = false;
		verwaltung.getAllActiveQuiz(new AsyncCallback<Vector<Quiz>>() {
		...
		if ((quizVector != null)	&& (quizVector.size() > 0)) {
			...
			for (Quiz q : quizVector) {
				...
				if(!q.isAutomatic() && !q.isStarted()) {
					...
					if (today != q.getStartingDate()) {
						timeLabel = new Label("Started am: " + DateTimeFormat.getFormat("yyyyMMdd").parse(new Integer(q.getStartingDate()).toString()).toString());
					}
					else {
						...
						if (q.isStarted()) {
							if (q.getDurationButton() > 0) {
								timeGap = ((q.getStartingTime() * 60) + q.getDurationButton()) - timeYet;
								if (timeGap <= 0) {
									startButton.setDisabled(true);
								}
							}
							if (q.getDurationButton() == 0) {
								timeGap = ((q.getStartingTime() * 60) + (q.getDurationQuestion() * q.getQuestionsCount())) - timeYet;
								if (timeGap <= 0) {
									startButton.setDisabled(true);
								}
							}
					...
					if (q.getDurationButton() > 0) {
						int timeGap = ((q.getStartingTime() * 60) + q.getDurationButton()) - timeYet;
						if (timeGap <= 0) {
							startButton.setDisabled(true);
						}
					}
					if (q.getDurationButton() == 0) {
						int timeGap = ((q.getStartingTime() * 60) + (q.getDurationQuestion() * q.getQuestionsCount())) - timeYet;
						if (timeGap <= 0) {
							startButton.setDisabled(true);
						}
					}
				...
				if (quizVector.size() > 0) {
					for(int i = 0; i < quizVector.size(); i++) {
						if (quizVector.elementAt(i).isStarted() && quizVector.elementAt(i).getDurationButton() > 0) {
							int timeGap = ((quizVector.elementAt(i).getStartingTime() * 60) + quizVector.elementAt(i).getDurationButton()) - timeYet;
							if (timeGap > 0) {
						...
						if (quizVector.elementAt(i).isStarted() && quizVector.elementAt(i).getDurationButton() == 0) {
							int timeGap = ((quizVector.elementAt(i).getStartingTime() * 60) + 
	...
	public void prepareQuiz() {
		...
		if (quizVector.elementAt(quizPos).getPassword() != null && !quizVector.elementAt(quizPos).getPassword().equals("")) {
			...
			verwaltung.getQuizCache(quizVector.elementAt(quizPos), new AsyncCallback<Void>() {
