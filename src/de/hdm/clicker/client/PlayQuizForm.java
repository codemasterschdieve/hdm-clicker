...
public class PlayQuizForm extends VerticalPanel {
	 ...
	 */	
	public PlayQuizForm(ControllerAsync verwaltungA) {
	...
	public void startQuiz() {
		qPointer = 0;
		questionLabel.setText(questionVector.elementAt(qOrder.elementAt(qPointer)).getQuestionText());
		initButtonsAndImg();
		...
		if (shownQuiz.getDurationButton() == 0) {
			entireTimeLeft = (shownQuiz.getStartingTime()*60 + (shownQuiz.getDurationQuestion()*questionVector.size())) - timeYet;
		}
		else {
			int buttonTime = shownQuiz.getStartingTime()*60 + shownQuiz.getDurationButton() - timeYet;
			if (buttonTime > 0) {
				entireTimeLeft = ((shownQuiz.getStartingTime()*60) + (shownQuiz.getDurationButton() - buttonTime) + (shownQuiz.getDurationQuestion()*questionVector.size())) - timeYet;
			}
			else {
				entireTimeLeft = ((shownQuiz.getStartingTime()*60) + shownQuiz.getDurationButton() + (shownQuiz.getDurationQuestion()*questionVector.size())) - timeYet;
			}
		}
	...
	public void sendAnswer() {
		isSending = true;
		verwaltung.getResults(result, new AsyncCallback<Boolean>() {
			@Override
	...
	public void sendAnswersLeft() {
		...
		verwaltung.getMissingResults(createResultsLeft(), new AsyncCallback<Vector<Boolean>>() {
			@Override
			public void onFailure(Throwable caught) {
				Window.alert(caught.getMessage());
				clicker.setChooseFormToMain();
	...
	public void initButtonsAndImg() {
		...
		if(questionVector.elementAt(qOrder.elementAt(qPointer)).getAnswer2Text() != null && 
				!questionVector.elementAt(qOrder.elementAt(qPointer)).getAnswer2Text().equals("")) {
			answersCount++;
		}
		if(questionVector.elementAt(qOrder.elementAt(qPointer)).getAnswer3Text() != null && 
				!questionVector.elementAt(qOrder.elementAt(qPointer)).getAnswer3Text().equals("")) {
			answersCount++;
		}
		if(questionVector.elementAt(qOrder.elementAt(qPointer)).getAnswer4Text() != null && 
				!questionVector.elementAt(qOrder.elementAt(qPointer)).getAnswer4Text().equals("")) {
			answersCount++;
		}
		...
		for (Integer bo : bOrder) {
			switch (bo) {
				case 0:
					...
					break;
				case 1:
					buttonFlexTable.setWidget(buttonFlexTable.getRowCount(), 0, createButton(questionVector.elementAt(qOrder.elementAt(qPointer)).getAnswer2Text()));
					break;
				case 2:
					buttonFlexTable.setWidget(buttonFlexTable.getRowCount(), 0, createButton(questionVector.elementAt(qOrder.elementAt(qPointer)).getAnswer3Text()));
					break;
				case 3:
					buttonFlexTable.setWidget(buttonFlexTable.getRowCount(), 0, createButton(questionVector.elementAt(qOrder.elementAt(qPointer)).getAnswer4Text()));
					break;
			}
		}
...
//			img.loadImg(""+questionVector.elementAt(qOrder.elementAt(qPointer)).getId());
	...
	public Vector<Result> createResultsLeft() {
		
		Vector<Result> vr = new Vector<Result>();
