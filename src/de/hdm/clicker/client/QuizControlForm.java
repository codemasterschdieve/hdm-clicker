...
public class QuizControlForm extends VerticalPanel {
	 ...
	 */	
	public QuizControlForm(ControllerAsync verwaltungA) {
	...
	public void ladenQuizze() {
		verwaltung.getAllQuizByTeacherAndActive(new AsyncCallback<Vector<Quiz>>() {
				...
				if(!q.isAutomatic() && !q.isStarted()) {
					...
					startButton.addClickHandler(new ClickHandler() {
						public void onClick(ClickEvent event) {
							...
							quizVector.elementAt(rowIndex).setStartingDate(new Integer(DateTimeFormat.getFormat("yyyyMMdd").format(new Date())));
							...
							verwaltung.startQuiz(quizVector.elementAt(rowIndex), new AsyncCallback<Quiz>() {
								...
								public void onSuccess(Quiz result) {
									...
									verwaltung.createNewQuizVersion(quizVector.elementAt(rowIndex), new AsyncCallback<Quiz>() {
					...
					if (new Integer(DateTimeFormat.getFormat("yyyyMMdd").format(new Date())) != q.getStartingDate()) {
					...
					for(int i = 0; i < labelVector.size(); i++) {
						if (integerVector.elementAt(i) > 0) {
							...
							for(int j = 0; j < quizVector.size(); j++) {
								...
								if (k == i) {
									verwaltung.startQuiz(quizVector.elementAt(quizVectorPointer), new AsyncCallback<Quiz>() {
										...
										public void onSuccess(Quiz result) {
											...
											verwaltung.createNewQuizVersion(quizVector.elementAt(quizVectorPointer), new AsyncCallback<Quiz>() {
