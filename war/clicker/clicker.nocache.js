...
(function($wnd, $doc){
  ...
  var badBrowser = 'Unable to load Super Dev Mode of "clicker" because\n';
  ...
  $wnd.__gwt_activeModules['clicker'] = {
    'moduleName' : 'clicker',
    'bindings' : function() {
      return {};
    }
 ...
 * "/clicker.nocache.js" is the one that loaded us. Normally this happens
...
function computeScriptBase() {
  ...
  $wnd.__gwt_activeModules['clicker'].superdevmode = true;

  var expectedSuffix = '/clicker.nocache.js';
  ...
  $wnd.alert('Unable to load Super Dev Mode version of ' + clicker + ".");
}
;
  ...
  var devModeHookKey = '__gwtDevModeHook:clicker';
  var devModeSessionKey = '__gwtDevModeSession:clicker';

  // Compute some codeserver urls so as the user does not need bookmarklets
  var hostName = $wnd.location.hostname;
  var serverUrl = 'http://' + hostName + ':9876';
  var nocacheUrl = serverUrl + '/clicker/clicker.nocache.js';
  ...
  $wnd.__gwt_activeModules['clicker'].canRedirect = true;
  $wnd.__gwt_activeModules['clicker'].superdevmode = true;
  ...
  $wnd[compileFunction] = function() {
    ...
    compileScript.src = serverUrl +
      '/recompile/clicker?user.agent=' + ua + '&_callback=' + callback;
